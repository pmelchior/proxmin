...
def plotResults(trace, label="", boundary=None):
    ...
    X = np.dstack(
        np.meshgrid(
            np.linspace(lims[0], lims[1], 1000), np.linspace(lims[0], lims[1], 1000)
        )
    )
    r = f(X)
    ...
    ax.text(
        0.05,
        0.95,
        "it %d: (%.3f, %.3f)" % (len(traj), traj[-1][0], traj[-1][1]),
...
if __name__ == "__main__":
    ...
    traceback.clear()
    proxmin.admm(
        X, prox_gradf, step_f, prox_g=prox, max_iter=max_iter, callback=traceback
    )
    ...
    traceback.clear()
    proxmin.admm(
        X, prox_direct, step_f, prox_g=None, max_iter=max_iter, callback=traceback
    )
    ...
    traceback.clear()
    proxmin.sdmm(
        X, prox_gradf, step_f, proxs_g=proxs_g, max_iter=max_iter, callback=traceback
    )
