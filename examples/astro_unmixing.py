...
def plotData(Y, A, S):
    ...
    try:
        ...
        ax0.text(
            0.05, 0.95, "Data", color="w", va="top", ha="left", transform=ax0.transAxes
        )
        ax0.set_xticks([])
        ax0.set_yticks([])
        ax1.text(
            0.05, 0.95, "Model", color="w", va="top", ha="left", transform=ax1.transAxes
        )
        ax1.set_xticks([])
        ax1.set_yticks([])
        ax2.text(
...
if __name__ == "__main__":
    ...
    components = [
        (generateAmplitude(flux, C), generateComponent(size, pos, N))
        for flux, size, pos in zip(fluxes, sizes, poss)
    ]
    ...
    meas_poss = (
        poss + np.random.normal(0, scale=np.ones(2)[None, :] * meas_sizes[:, None]) / 8
    )
    A0 = (
        np.sqrt(2 * np.pi)
        * meas_sizes ** 2
        * np.stack(
            [Y[:, N * posy + posx] for posy, posx in meas_poss.astype("int")], axis=1
        )
    )  # SED at peak location
    S0 = np.stack(
        [generateComponent(size, pos, N) for size, pos in zip(meas_sizes, meas_poss)],
        axis=0,
    )
    ...
    runs = (
        ...
        (
            proxmin.adaprox,
            dict(all_args, scheme="padam", b1=b1, b2=b2, prox_max_iter=100, p=0.45),
            "PAdam",
        ),
        (
            proxmin.adaprox,
            dict(all_args, scheme="amsgrad", b1=b1, b2=b2, prox_max_iter=100),
            "AMSGrad",
        ),
    )
